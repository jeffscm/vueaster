...
public class RenderTextureCamera : MonoBehaviour
{
	...
	public void MakeScreenWithPath(string path, System.Action onReady)
	{
		StartRenderingToTexture();  // Restart
		
		StartCoroutine(TakeScreenWithPath(path, onReady));
	}
	...
	private IEnumerator TakeScreenWithPath(string path, System.Action onReady)
	{
		yield return new WaitForEndOfFrame();
		...
		FrameTexture.Apply();
		saveImgWithPath(FrameTexture.EncodeToPNG(), path);
        yield return new WaitForEndOfFrame();
	...
	private string saveImgWithPath(byte[] imgPng, string path)
	{
